...
namespace UnityEditor.ProBuilder
{
    ...
    public sealed class ProBuilderEditor : EditorWindow, IHasCustomMenu
    {
        ...
        static void PrintMenuActionChild()
        {
            System.Reflection.Assembly assembly = System.Reflection.Assembly.Load("Unity.ProBuilder.Editor");
            System.Reflection.PropertyInfo pMenuTitle = typeof(MenuAction).GetProperty("menuTitle");

            System.Reflection.PropertyInfo pTooltip = typeof(MenuAction).GetProperty("tooltip");
            

            string s = "";
            foreach (var item in assembly.GetTypes())
                if (item.IsSubclassOf(typeof(MenuAction)))
                {
                    var obj = assembly.CreateInstance(item.FullName);
                    //s += "{\"" + pMenuTitle.GetValue(obj) + "\", \"\"},\n";
                    s += "{\"" + (pTooltip.GetValue(obj) as TooltipContent).title + "\", \"" + (pTooltip.GetValue(obj) as TooltipContent).summaryOir + "\"},\n";
                }
        ...
        //<<<<<<<<<<<<<<<<<<<<<<<
        [UserSetting("Toolbar", "Text Mode + Icon", "在 Text Mode 下显示图标。")]
        internal static Pref<bool> s_EnableTextModeIcon = new Pref<bool>("editor.toolbarTextModeIcon", false);

        [UserSetting("Toolbar", "Text Mode + ZH", "在 Text Mode 下使用中文标题。")]
        internal static Pref<bool> s_EnableTextModeZH = new Pref<bool>("editor.toolbarTextModeZH", false);
        //>>>>>>>>>>>>>>>>>>>>>>>
        ...
        //<<<<<<<<<<<<<<<<<<<<<<<<<
        void Menu_ToggleTextMode_Icon()
        {
            s_EnableTextModeIcon.value = s_IsIconGui | !s_EnableTextModeIcon.value;
            s_IsIconGui.value = false;
            IconModeChanged();
        }
        void Menu_ToggleTextMode_ZH()
        {
            s_EnableTextModeZH.value = s_IsIconGui | !s_EnableTextModeZH.value;
            s_IsIconGui.value = false;
            IconModeChanged();
        }
        //>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        public void AddItemsToMenu(GenericMenu menu)
        {
            ...
            menu.AddItem(new GUIContent("Text Mode + Icon", ""), !s_IsIconGui & s_EnableTextModeIcon,
                Menu_ToggleTextMode_Icon);
            menu.AddItem(new GUIContent("Text Mode + 汉化", ""), !s_IsIconGui & s_EnableTextModeZH,
